...
for apk in "$APK_DIR"/*.apk; do
    ...
    if [ -n "$ABI_LABEL" ]; then
        OUTPUT_NAME="zyxn-${VERSION}-${ABI_LABEL}.apk"
    else
        OUTPUT_NAME="zyxn-${VERSION}.apk"
    fi
...
if [ "$INSTALL" = true ]; then
    ...
    UNIVERSAL="$OUTPUT_DIR/zyxn-${VERSION}-universal.apk"
    if [ -f "$UNIVERSAL" ]; then
    ...
    echo "==> Launching app..."
    adb shell monkey -p io.zyxn -c android.intent.category.LAUNCHER 1 >/dev/null 2>&1
fi
