...
import io.zyxn.api.service.PluginLogger
...
/**
 * Represents a globally available service within the Zyxn ecosystem.
 *
 ...
 * [ZyxnPlugin] instance. These services provide context and lifecycle information
 ...
 *
 * These services are typically accessed via the `runtime()` delegate on a [ZyxnPlugin] instance:
 * ```kotlin
 ...
 *
 * This registry is responsible for mapping a [ZyxnPlugin] instance to its respective
...
/**
 * A delegate for accessing a [PluginRuntimeService] associated with a [ZyxnPlugin].
 *
 * Runtime services are provided by the zyxn only after a plugin is constructed and
 ...
 * [ZyxnPlugin.onLoad] / [ZyxnPlugin.onStart].
 *
...
) : ReadOnlyProperty<ZyxnPlugin, T> {

    private val cache = WeakHashMap<ZyxnPlugin, T>()

    @OptIn(InternalZyxnApi::class)
    override fun getValue(thisRef: ZyxnPlugin, property: KProperty<*>): T {
...
class PluginServiceDelegate<T : PluginService>(
    ...
    override fun getValue(thisRef: Any?, property: KProperty<*>): T {
        if (thisRef is ZyxnPlugin && service is Logger && clazz == Logger::class) {
