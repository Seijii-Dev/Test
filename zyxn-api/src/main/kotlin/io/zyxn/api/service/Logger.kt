 ...
 * logging context, such as [ZyxnPlugin]. For call sites with no meaningful
...
/**
 * Service responsible for logging events and messages within the Zyxn ecosystem.
 *
 ...
 *    If you have a `ZyxnPlugin` in context, you can use the concise extension:
 *    ```kotlin
 *    with(plugin) {
 ...
 *
 * The tag is derived from [ctx]: if it's a [ZyxnPlugin], the plugin's descriptor
...
fun Logger.log(level: LogLevel, message: String, throwable: Throwable? = null) {
    val tag = when (ctx) {
        is ZyxnPlugin -> ctx.info.descriptor.name
        NoLogContext -> "<anonymous>"
        else -> ctx::class.simpleName ?: "<anonymous>"
    }
    val sourcePluginId = (ctx as? ZyxnPlugin)?.info?.id
