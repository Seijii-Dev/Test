...
import io.zyxn.compiler.plugin.ZyxnPluginIds
...
object ZyxnManifestErrors : KtDiagnosticsContainer() {
    ...
    val NOT_A_ZYXN_PLUGIN by error0<KtElement>()
    ...
    override fun getRendererFactory(): BaseDiagnosticRendererFactory = ZyxnManifestDiagnosticRendererFactory()

    private class ZyxnManifestDiagnosticRendererFactory : BaseDiagnosticRendererFactory() {
        ...
        override val MAP by KtDiagnosticFactoryToRendererMap("PluginManifest") { map ->
            map.apply {
                ...
                put(NOT_A_ZYXN_PLUGIN, "'@PluginManifest' can only annotate a class implementing ZyxnPlugin")
                ...
                put(
                    ...
                    "Class ''{0}'' implements ZyxnPlugin but is missing ''@PluginManifest(...)''. " +
                            "Every ZyxnPlugin implementation must be annotated.",
                    rendererA = TO_STRING
                )
                put(
...
internal val manifestPredicate: LookupPredicate = LookupPredicate.create {
    annotated(ZyxnPluginIds.PLUGIN_MANIFEST_FQN)
}

class ZyxnManifestCheckers(session: FirSession) : FirAdditionalCheckersExtension(session) {

    override val declarationCheckers = object : DeclarationCheckers() {
        override val classCheckers: Set<FirClassChecker> = setOf(
            PluginManifestClassChecker,
            ZyxnPluginImplementationChecker
        )
    }
...
private object ZyxnPluginImplementationChecker : FirClassChecker(MppCheckerKind.Common) {

    context(context: CheckerContext, reporter: DiagnosticReporter)
    override fun check(declaration: FirClass) {
        val implementsZyxnPlugin = declaration.superConeTypes.any {
            it.classId == ZyxnPluginIds.ZYXN_PLUGIN_CLASS_ID
        }
        if (!implementsZyxnPlugin) return

        val hasManifest = declaration.annotations.any {
            it.resolvedType.classId == ZyxnPluginIds.PLUGIN_MANIFEST_CLASS_ID
        }

        if (!hasManifest) {
            val source = declaration.source ?: return
            reporter.reportOn(source, ZyxnManifestErrors.MISSING_PLUGIN_MANIFEST, declaration.classId.shortClassName)
        }
    }
}
...
private object PluginManifestClassChecker : FirClassChecker(MppCheckerKind.Common) {
    ...
    override fun check(declaration: FirClass) {
        ...
        if (id == null || !ZyxnPluginIds.isValidPluginId(id)) {
            reporter.reportOn(sourceFor("id"), ZyxnManifestErrors.INVALID_PLUGIN_ID, id.orEmpty())
        }
        ...
        listOfNotNull(
            ...
            if (!ZyxnPluginIds.isValidSemver(value)) {
                reporter.reportOn(sourceFor(label), ZyxnManifestErrors.INVALID_SEMVER, label, value)
            }
        }
        ...
        if (
            ...
            ZyxnPluginIds.isValidSemver(minAppVersion) &&
            ZyxnPluginIds.isValidSemver(maxAppVersion)
        ) {
            if (compareSemver(minAppVersion, maxAppVersion) > 0) {
                reporter.reportOn(
                    sourceFor("minAppVersion"),
                    ZyxnManifestErrors.MIN_VERSION_EXCEEDS_MAX,
                    minAppVersion,
                    maxAppVersion
                )
            }
        ...
        if (name.isNullOrBlank()) {
            reporter.reportOn(sourceFor("name"), ZyxnManifestErrors.BLANK_NAME_FALLS_BACK_TO_ID, id)
        }
        ...
        if (!implementsZyxnPlugin) {
            reporter.reportOn(classSource, ZyxnManifestErrors.NOT_A_ZYXN_PLUGIN)
        }
        ...
        if (declaration is FirRegularClass) {
            ...
            if (badModifiers.isNotEmpty()) {
                reporter.reportOn(
                    classSource, ZyxnManifestErrors.INVALID_ENTRY_CLASS_MODIFIERS,
        ...
        val ownDescriptor = declaration.declarations
            ...
            ?.firstOrNull { it.symbol.callableId?.callableName == ZyxnPluginIds.DESCRIPTOR_PROPERTY_NAME }

        if (ownDescriptor != null) {
            reporter.reportOn(ownDescriptor.source ?: classSource, ZyxnManifestErrors.RESERVED_DESCRIPTOR_NAME)
        }
        ...
        if (!hasPublicNoArgConstructor) {
            reporter.reportOn(classSource, ZyxnManifestErrors.MISSING_NO_ARG_CONSTRUCTOR)
        }
        ...
        if (allManifestClasses.size > 1) {
            ...
            reporter.reportOn(classSource, ZyxnManifestErrors.DUPLICATE_PLUGIN_MANIFEST, others)
        }
    }
}
